...
.PHONY: all clean test mdtools coverage

all: | bin $(TARGET) mdtools
...
clean:
	rm -rf bin/cov
	rm -f $(OBJ) $(TARGET) $(TEST_BIN) $(MDTOOLS_BIN) bin/*.gcda bin/*.gcno bin/*.gcov
...
mdtools: $(TARGET)
	$(CC) $(CFLAGS) -o $(MDTOOLS_BIN) examples/mdtools.c $(TARGET)
	@echo "Tool built: $(MDTOOLS_BIN)"
	@echo "Run with: ./$(MDTOOLS_BIN)"
